...
export default class BiPspbReminderSetup extends LightningElement {
	...
	handleDateChange(event) {
		...
		const DIFFERENCE_DAYS = Math.floor(
		    ...
		    const dobField = this.template.querySelector('input[data-field="DOB"]');
    const dobLabel = this.template.querySelector('p[data-field="DOB"]');
	if (this.selectedDate) {
        if (dobField) {
            dobField.className = "textInput"; // Apply error class
        }
        if (dobLabel) {
            dobLabel.className = "input-label"; // Apply error class
        }
        this.dobErrorMessage = false; // Set error message flag
...
const DOB_FIELD = this.template.querySelector('input[data-field="DOB"]');

    // Validate the DOB field
    if (!DOB_FIELD || !DOB_FIELD.value) {
        if (DOB_FIELD) {
            DOB_FIELD.className = "textInput-err"; // Apply error class
        }
        this.dobErrorMessage = true; // Set error message flag
        const dobLabel = this.template.querySelector('p[data-field="DOB"]');
        if (dobLabel) {
            dobLabel.className = "input-error-label"; // Apply error class to DOB label
        }
        this.isSubmitButtonDisabled = false; // Re-enable the button if validation fails
        return; // Exit the function if validation fails
    } else {
        // Clear error styles if DOB_FIELD has a value
        if (DOB_FIELD) {
            DOB_FIELD.className = "textInput"; // Reset to normal class
        }
        this.dobErrorMessage = false; // Clear error message flag
        const dobLabel = this.template.querySelector('p[data-field="DOB"]');
        if (dobLabel) {
            dobLabel.className = "input-label"; // Reset DOB label class
        }
    }
